...
```sh
helm install <RELEASE_NAME> cloudzero/cloudzero-agent --create-namespace -f configuration.example.yaml --devel
```
...
```sh
helm install <RELEASE_NAME> cloudzero/cloudzero-agent --create-namespace -f configuration.example.yaml --version <CHART_VERSION>
```
